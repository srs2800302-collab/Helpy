...
class _ClientHomeScreenState extends ConsumerState<ClientHomeScreen> {
  static const _readClientMessageTimestampsKey =
  ...
  Future<void> _loadReadMessageTimestamps() async {
    final items =
        await loadReadMessageTimestamps(_readClientMessageTimestampsKey);
  ...
  Future<void> _markMessageRead(String jobId, DateTime? createdAt) async {
    final next = await markReadMessageTimestamp(
      keys: const [
        _readClientMessageTimestampsKey,
  ...
  Widget build(BuildContext context) {
    ...
    final visibleJobs = jobsState.items
        .where((item) => !(item.status == 'completed' &&
            _hiddenCompletedJobIds.contains(item.id)))
        .toList();
    final jobsWithOffers = visibleJobs
        .where((item) => item.status == 'open' && item.offersCount > 0)
        .toList();
    final incomingMessageJobs = visibleJobs.where((item) {
          ...
          children: [
            ...
            if (jobsWithOffers.isNotEmpty) ...[
              Card(
                ...
                child: Column(
                  children: jobsWithOffers.map((job) {
                    ...
                    return ListTile(
                      ...
                      subtitle: Column(
                        ...
                        children: [
                          ...
                          Text(l10n.t('offers_count').replaceAll(
                              '{count}', job.offersCount.toString())),
                        ],
                      ),
                      isThreeLine: false,
            ...
            if (incomingMessageJobs.isNotEmpty) ...[
              Card(
                ...
                child: Column(
                  children: incomingMessageJobs.map((job) {
                    final title = (job.titleOriginal ?? job.title).trim();
                    final displayTitle = translatedOrOriginal(
                      original: title,
                      translationsJson: job.titleTranslationsJson,
                    ...
                    return ListTile(
                      ...
                      onTap: () async {
                        await _markMessageRead(
                            job.id, job.lastMessageCreatedAt);
