SELECT
  (SELECT COUNT(*) FROM offers WHERE status NOT IN ('active', 'selected', 'rejected', 'cancelled')) AS offers_invalid_status,
  (SELECT COUNT(*) FROM offers WHERE status = 'selected') AS offers_selected_status,
  (SELECT COUNT(*) FROM offers WHERE status = 'accepted') AS offers_legacy_accepted_status,
