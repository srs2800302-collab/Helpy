...
function sanitizeOffer(row: any) {
  ...
  return {
    ...
    message_translations_json: row.message_translations_json ?? null,
    created_at: row.created_at,
    master_user_id: row.master_user_id,
    job_title: row.job_title ?? '',
    job_title_original: row.job_title_original ?? row.job_title ?? '',
...
export async function getOffersByMaster(
  ...
  const result = await env.DB.prepare(
    `SELECT
       ...
       o.message,
       o.message_translations_json,
       o.created_at,
       o.master_user_id,
       j.title AS job_title,
       j.title_original AS job_title_original,
