...
export async function handleRequest(request: Request, env: any) {
  ...
  if (url.pathname.match(/^\/api\/v1\/users\/[^/]+$/) && request.method === 'GET') {
    const id = url.pathname.split('/')[4];
    return getUser(id, env);
  }

  // client profile
  if (url.pathname.match(/^\/api\/v1\/users\/[^/]+\/client-profile$/) && request.method === 'POST') {
    const userId = url.pathname.split('/')[4];
    return createClientProfile(userId, request, env);
  }

  // master profile
  if (url.pathname.match(/^\/api\/v1\/users\/[^/]+\/master-profile$/) && request.method === 'POST') {
    const userId = url.pathname.split('/')[4];
    return createMasterProfile(userId, request, env);
  }

  // jobs
  if (url.pathname === '/api/v1/jobs' && request.method === 'GET') return getJobs(env);
  if (url.pathname === '/api/v1/jobs' && request.method === 'POST') return createJob(request, env);
  ...
  if (url.pathname.match(/^\/api\/v1\/jobs\/[^/]+\/status$/) && request.method === 'PATCH') {
    const jobId = url.pathname.split('/')[4];
  ...
  if (url.pathname.match(/^\/api\/v1\/jobs\/[^/]+\/select-offer$/) && request.method === 'POST') {
    const jobId = url.pathname.split('/')[4];
    return selectOffer(jobId, request, env);
  }

  if (url.pathname.match(/^\/api\/v1\/jobs\/[^/]+\/complete$/) && request.method === 'POST') {
    const jobId = url.pathname.split('/')[4];
    return completeJob(jobId, env);
  }

  if (url.pathname.match(/^\/api\/v1\/jobs\/[^/]+\/reviews$/) && request.method === 'GET') {
    const jobId = url.pathname.split('/')[4];
    return getReviews(jobId, env);
  }

  if (url.pathname.match(/^\/api\/v1\/jobs\/[^/]+\/reviews$/) && request.method === 'POST') {
    const jobId = url.pathname.split('/')[4];
