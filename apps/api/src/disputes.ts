...
import { ensureDisputesSchema, canCreateDisputeInStatus } from './disputes.helpers';
...
export async function createDispute(jobId: string, request: Request, env: any) {
  ...
  if (!canCreateDisputeInStatus(job.status, JOB_STATUS)) {
    return Response.json(
      {
        success: false,
  ...
  await env.DB.prepare(
    ...
    .bind(id, jobId, actorUserId, body.reason.trim(), 'open', null, null, null, now)
    .run();
