...
async function createOffer(jobId, request, env) {
    ...
    const profile = await env.DB.prepare(`SELECT mp.*, u.language AS user_language
     FROM master_profiles mp
     LEFT JOIN users u ON u.id = mp.user_id
     WHERE mp.user_id = ?1`).bind(masterUserId).first();
    if (!profile)
    ...
    try {
        ...
        const commentTranslationsJson = commentText
            ? await (0, translation_1.buildTranslationsJson)({
                text: commentText,
                sourceLanguage,
                env,
                entityType: 'offer',
        ...
        created_at
            ...
            .bind(id, jobId, masterUserId, body.master_name, body.price, commentText || null, messageText || null, commentTranslationsJson, messageTranslationsJson, now)
            .run();
        ...
        const createdOffer = await env.DB.prepare('SELECT * FROM offers WHERE id = ?1').bind(id).first();
