...
exports.enableMasterBillingForTest = enableMasterBillingForTest;
...
async function enableMasterBillingForTest(masterUserId, request, env) {
    ...
    if (auth.role !== 'admin') {
        return Response.json({ success: false, error: 'Only admin can enable test billing' }, { status: 403 });
    }
    const profile = await env.DB.prepare('SELECT user_id FROM master_profiles WHERE user_id = ?1 LIMIT 1')
        .bind(masterUserId)
        .first();
    if (!profile) {
        return Response.json({ success: false, error: 'Master profile not found' }, { status: 404 });
    }
...
//# sourceMappingURL=master-billing-test.js.map