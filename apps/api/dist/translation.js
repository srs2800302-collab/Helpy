...
exports.buildTranslationsJson = buildTranslationsJson;
exports.cleanupTranslationTasksForEntity = cleanupTranslationTasksForEntity;
exports.processPendingTranslationTasks = processPendingTranslationTasks;
const SUPPORTED_LANGUAGES = ['ru', 'en', 'th'];
...
async function ensureTranslationTasksSchema(env) {
...
function buildEmptyTranslations(originalText, sourceLanguage) {
    const entries = {
        ru: '',
        en: '',
        th: '',
...
async function buildTranslationsJson({ text, sourceLanguage, env, entityType, entityId, fieldName, }) {
    ...
    `).bind(`${entityType}:${entityId}:${fieldName}:${source}:${target}`, entityType, entityId, fieldName, source, target, originalText, now, now).run();
    }
    return JSON.stringify(translations);
}
async function cleanupTranslationTasksForEntity({ env, entityType, entityId, }) {
    await ensureTranslationTasksSchema(env);
    await env.DB.prepare('DELETE FROM translation_tasks WHERE entity_type = ?1 AND entity_id = ?2').bind(entityType, entityId).run();
}
async function translateWithGooglePublic({ text, sourceLanguage, targetLanguage, }) {
  ...
  `;
    ...
    for (const task of tasks) {
        try {
            ...
            const table = entityTypeValue === 'job'
                ? 'jobs'
                : entityTypeValue === 'offer'
                    ...
                    : entityTypeValue === 'chat_message'
                        ? 'chat_messages'
                        : null;
            const column = entityTypeValue === 'job' && fieldName === 'title'
