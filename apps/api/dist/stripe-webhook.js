...
const stripe_event_utils_1 = require("./stripe-event-utils");
...
async function handleStripeWebhook(request, env) {
    ...
    const summary = (0, stripe_event_utils_1.extractStripeEventSummary)(event);
    ...
    const finalStatus = eventType === 'setup_intent.succeeded' &&
        summary.customerId &&
    ...
    const existing = await env.DB.prepare(`SELECT id, provider, provider_event_id, event_type, customer_id, payment_method_id, status, created_at, processed_at
     FROM payment_events
    ...
    await env.DB.prepare(`INSERT INTO payment_events (
      ...
      processed_at
        ...
        .bind(id, 'stripe', providerEventId, eventType, objectType, objectId, summary.customerId, summary.paymentMethodId, payloadText, 'received', now, null)
        .run();
    const processedAt = new Date().toISOString();
    await env.DB.prepare(`UPDATE payment_events
     SET status = ?2,
         processed_at = ?3
     WHERE id = ?1`)
