...
Plumbing
├── Кран / Смеситель 🟨 STORED PARTIALLY
├── Засор ✅ STORED + DOCS
├── Унитаз ✅ STORED + DOCS
├── Проточный водонагреватель ✅ STORED + DOCS
└── Другая проблема 🟨 STORED PARTIALLY
...
- Где установлен?
  ...
  - Кухня
  - Другое
- Опишите проблему своими словами — обязательно для сценария «Другое»
...
Known Rules:
- Клиент не обязан понимать технические причины поломки.
- Не использовать клиентский термин «закис».
- Клиент выбирает наблюдаемую проблему: не работает, течет, нужно установить или заменить.
- Замена смесителя подразумевает демонтаж старого и установку нового.
- Установка нового смесителя без демонтажа является отдельным сценарием.
- Материалы, переходники, шланги и поездка за материалами обсуждаются отдельно в чате.
...
Required Photos:
- минимум 3 фотографии
- до 10 фотографий при сложной или неизвестной проблеме

Known Rules:
- Используется, если клиент не может отнести проблему к существующим подкатегориям.
- Клиент обязан описать проблему текстом.
- Клиент обязан приложить фотографии.
- Если клиент вообще не понимает источник проблемы, должна использоваться глобальная услуга диагностики.
- Диагностика является отдельной услугой платформы.
- Глобальная диагностика — от 800 ฿.
